...
`downlink` 配置项决定 `/api/events.mux` 与 `/api/events.host` 以何种方式抵达客户端，两个半侧必须取同一个值。

`websocket`（默认）是浏览器载体。两条路径各接受一条 WebSocket upgrade，并只向浏览器发送对应的 `ServerRequest` 文本消息；客户端不会在这些 socket 上发送业务数据。任一 socket 结束都会使当前 connection generation 失败并重建两条流，连接就绪仍要求两条 socket 均已打开且 `host.describe` HTTP 调用成功。Host teardown 会终止两条 socket、中止各自的 source，并等待 source 清理完成后再返回。普通网络 GET 这些路径会返回 426，不保留 SSE（Server-Sent Events）回退。

`sse` 面向没有 HTTP upgrade 的载体，例如 [electron-carrier](../../host/electron-carrier/README.md)。GET 会落到 `toFetchHandler` 的 SSE 编解码——与流式响应是同一批 frame——并且完全不注册 upgrade 路由，因此具备 upgrade 能力的载体不会为同一批流额外提供一条无人使用的路径。客户端半侧自行选择匹配的读取端：自定义 scheme 上的页面构造不出 `ws:`/`wss:` URL，因此使用 `SseApiClient`，其 `openMux`/`openHost` 就是 `AbstractApiClient` 读 SSE 的默认实现。
