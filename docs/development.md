...
- Node.js supports 22.19+ and 24+. CI covers 22.19, 24, and 26; see the [Node engine floor Agent Note](../.agents/notes/implemented/process/2026-07-06-node-engine-floor.md). `.nvmrc` pins CI's primary version for `nvm use`, because an older Node fails the pre-commit hooks inside their runner rather than at an engines check.
